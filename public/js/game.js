...
async function makeGuess(letter, gameState) {
	...
	if (!gameState.guessedLetters.includes(letter)) {
		...
		if (!gameState.wordToGuess.includes(letter)) {
			...
			if (
				gameState.wordToGuess.every((l) => gameState.guessedLetters.includes(l))
			) {
				gameState.score += 250;
				alert("You guessed the word! Your score is " + gameState.score);

				let high_scores = gameState.score > 0 ? gameState.score : 0;
				const response = fetch("/api/scores", {
					method: "POST",
					body: JSON.stringify({ high_scores }),
					headers: {
...
function updateGame(gameState) {
	...
	document
		.querySelector(".head")
		.classList.toggle("hidden", gameState.incorrectGuesses < 1);
	document
		.querySelector(".body")
		.classList.toggle("hidden", gameState.incorrectGuesses < 2);
	document
		.querySelector(".left-arm")
		.classList.toggle("hidden", gameState.incorrectGuesses < 3);
	document
		.querySelector(".right-arm")
		.classList.toggle("hidden", gameState.incorrectGuesses < 4);
	document
		.querySelector(".left-leg")
		.classList.toggle("hidden", gameState.incorrectGuesses < 5);
	document
		.querySelector(".right-leg")
		.classList.toggle("hidden", gameState.incorrectGuesses < 6);

	document.getElementById("incorrect-guesses").textContent =
		`Incorrect Guesses: ${gameState.incorrectGuesses}`;
